...
import {
    ApiSchemaSourceDTO, FormFieldDTO, getMetaProvider, RoutesMetadataDTO, ItemSchemaDTO,
    RouteMetadataDTO,
    SchemaTypeDTO,
    SchemaDTO,
} from "@tsdiapi/meta";
...
export default async function registerMetaRoutes({ useRoute }: AppContext) {
    ...
    useRoute("meta")
        ...
        .code(200, Type.Array(SchemaTypeDTO))
        .handler(async (req) => {
            const data = await meta.getAllTypesFromRoute(req.query.route, req.query.method, req.params.type as SchemaType);
